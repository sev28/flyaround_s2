<?php
// src/Flyaround/DefaultBundle/TerrainController.php
...
class TerrainController extends FOSRestController
{
    ...
    public function getTerrainsAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        if ($lat1 && $lat2 && $lng1 && $lng2)
            $entities = $em->getRepository('FlyaroundDefaultBundle:Terrain')->getZone($lat1, $lat2, $lng1, $lng2);
        else
            $entities = $em->getRepository('FlyaroundDefaultBundle:Terrain')->findByCountry('France');
